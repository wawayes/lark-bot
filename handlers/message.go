...
func (f *HandlerFactory) CreateHandler(messageType string) MessageHandler {
	switch messageType {
	case "text":
		return f.CreateTextHandler(&BaseHandler{Logger: f.Logger, BotHelper: f.BotHelper, LarkClient: f.LarkClient, BotHandlers: f.BotHandlers}, f.QweatherClient, f.LocationService)
	case "image":
